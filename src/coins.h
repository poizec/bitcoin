 ...
 * CCoinsViewCache::Sync and an erasing CCoinsViewCache::Flush. This allows the callee
 * of CCoinsView::BatchWrite to iterate through the flagged entries without knowing
 * the caller's intent.
 *
 * However, the callee can still call CoinsViewCacheCursor::WillErase to see if the
 * caller will erase the entry after BatchWrite returns. If so, the callee can
