...
class ReplaceByFeeTest(BitcoinTestFramework):
    ...
    def test_simple_doublespend(self):
        ...
        reject_details = f"rejecting replacement {tx.hash}; new feerate 0.00300000 BTC/kvB <= old feerate 0.00300000 BTC/kvB"
        assert_raises_rpc_error(-26, f"{reject_reason}, {reject_details}", self.nodes[0].sendrawtransaction, tx_hex, 0)

        res = self.nodes[0].testmempoolaccept(rawtxs=[tx_hex])[0]
        assert res["reject-reason"] == reject_reason
        assert res["reject-details"] == reject_details
    ...
    def test_doublespend_chain(self):
        ...
        # This will raise an exception due to insufficient fee
        reject_reason = "insufficient fee"
        reject_details = f"rejecting replacement {dbl_tx.hash}, less fees than conflicting txs; 3.00 < 4.00"
        assert_raises_rpc_error(-26, f"{reject_reason}, {reject_details}", self.nodes[0].sendrawtransaction, dbl_tx_hex, 0)
    ...
    def test_spends_of_conflicting_outputs(self):
        ...
        reject_details = f"{tx2.hash} spends conflicting transaction {tx1a['tx'].hash}"
        assert_raises_rpc_error(-26, f"{reject_reason}, {reject_details}", self.nodes[0].sendrawtransaction, tx2_hex, 0)
    ...
    def test_new_unconfirmed_inputs(self):
        ...
        reject_details = f"replacement {tx2.hash} adds unconfirmed input, idx 1"
        assert_raises_rpc_error(-26, f"{reject_reason}, {reject_details}", self.nodes[0].sendrawtransaction, tx2_hex, 0)
    ...
    def test_too_many_replacements(self):
        ...
        reject_details = f"rejecting replacement {double_tx.hash}; too many potential replacements ({MAX_REPLACEMENT_LIMIT + 1} > {MAX_REPLACEMENT_LIMIT})"
        assert_raises_rpc_error(-26, f"{reject_reason}, {reject_details}", self.nodes[0].sendrawtransaction, double_tx_hex, 0)
